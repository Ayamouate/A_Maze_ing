...
class Maze:
    ...
    def prim_algo(self) -> None:
        ...
        # Add all neighbors and walls of the entry cell to the wall list
        for nx, ny, dir, opp in self.find_unvisited_neighbors(ex, ey):
            walls.append((ex, ey, nx, ny, dir, opp))

        while walls:
            # Pick a random wall or cell
            idx = random.randint(0, len(walls) - 1)
            x, y, nx, ny, direction, opposite = walls.pop(idx)
            ...
            if self.visited[ny][nx] == 0:
                ...
                for nnx, nny, nd, no in self.find_unvisited_neighbors(nx, ny):
                    if self.visited[nny][nnx] == 0:
